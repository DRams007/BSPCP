...
const FloatingDonationWidget = () => {
  ...
  if (isMinimized) {
    return <div className="fixed bottom-6 right-6 z-50">
        <Button onClick={() => setIsMinimized(false)} className="rounded-full w-16 h-16 bg-red-500 hover:bg-red-600 shadow-lg hover:shadow-xl transition-all duration-300 group" size="icon">
          <Heart className="w-8 h-8 text-white animate-pulse" fill="currentColor" />
        </Button>
      </div>;
  }
  return <div className="fixed bottom-6 right-6 z-50 max-w-sm">
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow-xl border border-border p-4 transform transition-all duration-300 hover:scale-105">
        <div className="flex items-start justify-between mb-3">
          ...
          <Button variant="ghost" size="sm" onClick={() => setIsMinimized(true)} className="h-8 w-8 p-0 hover:bg-muted">
            <X className="w-4 h-4" />
          </Button>
        </div>
        ...
        <div className="space-y-2">
          <Button onClick={scrollToDonation} className="w-full text-sm text-white bg-green-600 hover:bg-green-500">
